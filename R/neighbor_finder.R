...
neighbor_finder <- function(pid, pop_reg, house_reg, dist_radius=50){
    ...
    if(is.numeric(dist_radius) & dist_radius <= 0) stop("dist_radius must be > 0")
    if(!is.numeric(house_reg$x_coord)) stop("house_reg$x_coord must be numeric")
    if(!is.numeric(house_reg$y_coord)) stop("house_reg$y_coord must be numeric")
    if(!pid %in% pop_reg$pid) stop("pid is not in pop_reg")

    if(any(!is.na(pop_reg$household) & is.na(pop_reg$village))) stop("pop_reg has household with no villages - dangerous!")

    my_household <- pop_reg$household[which(pop_reg$pid==pid)]
    if(is.na(my_household)) stop("pid's household is NA")
    if(!my_household %in% house_reg$household) stop("pid's household is not in the house_reg")

    my_village <- pop_reg$village[which(pop_reg$pid==pid)]
    my_house_x_coord <- house_reg$x_coord[which(house_reg$household==my_household)]
    my_house_y_coord <- house_reg$y_coord[which(house_reg$household==my_household)]
